...
---
## 1655, 가운데를 말해요 
최소 우선순위큐, 최대 우선순위 큐를 적절히 이용해서 풀었던 문제.  
   
최소 우선순위큐 (min)과 최대 우선순위큐 (max)의 균형을 맞춰주면서 mid 값을 구해줬음.  

중간값을 mid에 저장해둔 후 mid보다 작은 값들은 최대 우선순위큐 (max), mid보다 큰값들은 최소 우선순위큐 (min)에 넣어 주는 방식으로 mid 값을 구해줬음.  -> 큐들의 크기가 항상 1 차이 나거나 같아짐

1. 만약 큰 값들의 개수가 (min큐) 작은값들의 개수보다 작거나 같다면 (max큐)  
    <strong>(1)</strong> 들어오는 값(temp)이 mid보다 크거나 같다면 temp를 min큐에 넣어줌 -> 균형 맞아짐(만약 크기 차이가 0인 상태였다면 균형은 꺠지지만 mid값은 조건에 맞게 작은 값이 오게됨)
     
    <strong>(2)</strong> 들어오는 값(temp)이 mid보다 작다면 mid를 min큐에 넣은 후 max큐에 temp를 넣은 후 mid 값을 max큐에서 poll한 값으로 설정함 (기존의 mid는 이 순간 큰 값들의 집함으로 들어감)
     ->  temp 값이 max큐에 들어가야하는데 균형이 max큐가 더 커지므로 균형이 꺠짐. 따라서 mid 값이 바껴야함  
     -> max큐가 더 크므로 max큐에서 poll한 값을 mid로 설정 
2. 만약 큰 값들의 개수가 작은값들의 개수보다 크다면 
    <strong>(1)</strong> temp가 mid보다 크다면 큰값들의 개수가 더 커지므로 균형이 깨짐  
     -> 따라서 이 순간 mid값은 작은값들의 집합으로 들어가게 되고 mid값을 max큐에 넣어줌, temp값은 min값에 넣어주고 mid값을 min 큐에서 poll한 값을 설정함으로써 균형을 맞춰줌
     <strong>(2)</strong>temp가 mid보다 작거나 같다면 temp가 max큐에 들어가야 함.  
     -> 균형이 맞으므로 mid값을 변경하지 않음. 
    
조금 복잡하지만 그림을 그려 가며 풀면 메커니즘이 이해 될 것. 

---
## 21939, 문제 추천 시스템 version1 
정렬이 필요하고 문제 갯수가 최대 100,000 이므로 nlogn으로 정렬이 가능한 우선 순위 큐를 사용했음.  
  
최대 우선순위큐를 사용해 recommend 1을 처리하고 최소 우선순위 큐를 사용해 recommend -1을 처리함.  
모든 문제들을 두 개의 우선순위큐에 모두 넣어서 처리하도록 함.  
  
우선 순위큐의 객체 삭제 알고리즘이 O(n) 이므로 해보진 않았지만 아마 remove 메서드를 사용했다면 시간 초과가 나지 않을 까 해서 삭제하는 경우 Set을 이용해 삭제함.  

1. add를 하는 경우 set에 Problem을 넣어줌.  
2. solved인 경우 remove 메서드를 사용하지 않고 set와 map에 problem을 삭제해줌 (이때, map은 problem 객체를 얻기위해서 사용함)
3. recommend 할 때 set에 problem이 있는 지 체크하고 set에 있다면 print해줌 -> set에 없다면 poll해주며 set에 있는 problem이 나올 떄 까지 반복. 

위와 같은 방법으로 삭제할 때 걸리는 시간을 줄여줬음. remove 메서드를 사용하면 시간초과 걸리는지는 확인안해봄.  

---
## 21944, 문제 추천 시스템 Version 2 
구현하는데 1시간 정도 걸린 문제.  
문제 조건 그대로 구현하고 수정하는데에 많은 시간을 쏟았음.  

기본적인 메커니즘은 <strong>21939, 문제 추천 시스템 Version 1</strong>을 토대로 구현하고 추가적으로 요구하는 구현사항만 따로 추가함.  
난이도, 알고리즘 그룹이 최대 100개이므로 메모리 문제, 시간 문제가 발생할것 같지 않아 각각 100개의 우선순위큐를 만들어 문제를 풀었음.
그래서 코드가 좀 지저분해 진 것 같음.  

다른 사람 풀이를 보니 TreeSet , TreeMap을 사용해서 깔끔하게 풀었던데 솔직히 이해가 잘 가지 않음.   
TreeSet, TreeMap을 이용하는 문제를 많이 풀어야 겠다고 느꼈음.  

----
## 21942, 물품 대여장
월, 일, 시간을 전부 분단위로 변환한 후 대여시간과 비교하면 됐던 문제.  
<물품목록, 대여시간> 을 기록하는 Map과 <사람, 벌금>을 기록하는 Map 을 사용하여 문제 조건에 맞게 구현했음  
  
예외 케이스 떄문에 시간이 오래걸렸는데, 내가 찾은 예외 케이스는 같은 물건(종류)를 여러 사람이 대여할 수 있는 케이스였음.  
예를들면 Arduino 물건을 A,B,C 가 대여가능했음.  
<물품 목록, 대여시간> 을 기록하는 Map의 key값을 물품목록+사람이름 으로 설정해서 여러사람이 같은 물건을 대여했을떄도 계산이 가능하도록 설정함.  
   
문자열 파싱하는 부분, 분단위로 변환하는 함수 구현하는게 귀찮긴 했지만 어렵지는 않았음.  
